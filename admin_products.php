...
function isSell($product_name) // $street
{

  global $conn;
  $select_total_products = mysqli_query($conn, "SELECT * FROM `orders`")  or die('query failed');

  if (mysqli_num_rows($select_total_products) > 0) {
    while ($fetch_total_products = mysqli_fetch_assoc($select_total_products)) {

      if ((strpos($fetch_total_products['total_products'], $product_name)) !== false) {
        // echo 'NAISU';
        return true;
      }
    }
    ...
    <div class="box-container">
      ...
      if (mysqli_num_rows($select_products) > 0) {
        while ($fetch_products = mysqli_fetch_assoc($select_products)) {
          if (isSell($fetch_products['name']) == false) {
      ?>
      <div class="box">
