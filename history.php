...
-- ORDER BY place_on DESC";
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      echo '<thead><tr><th>Order ID</th><th>Order date</th><th>Products</th><th>Total</th><th>Status</th></tr></thead>';
      echo '<tbody>';
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<td class="order-products">' . $row["total_products"] . '</td>';
        echo '<td class="order-price">' . $row["total_price"] . '</td>';
        echo '<td class="order-status">' . $row["payment_status"] . '</td>';
        echo '</tr>';
      }
      echo '</tbody></table>';
    } else {
